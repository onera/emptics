...
int main(int argc, char *argv[]) {
  ...
  if (mpi_size < 2) {
    printf("The HDF5 parallel test should be launched with at least two processes\n");
    return 1;
  }
   ...
   */
  const char* H5FILE_NAME = "SDS.h5";
  const char* DATASETNAME = "IntArray";
  hsize_t NX = 8; // dataset dimensions
  hsize_t NY = 5;
  int RANK = 2;
  ...
  if (mpi_rank == 0) printf("PHDF5 example finished with no errors\n");
